...
| 需求                                                         | 选型         | 备注                                                         |
| ------------------------------------------------------------ | ------------ | ------------------------------------------------------------ |
| schema 不确定，由外部系统决定；<br />存在许多不同类型的对象，将每种类型的对象放在自己的表中是不现实的；<br />应用程序经常需要访问整个文档（例如，将其渲染至网页）; | mongodb      | 例如医疗大数据中的处方，就适合用 mongodb 存储；mongodb 的单个是连续存储在磁盘上的，适合海量数据处理，OLAP。 |
